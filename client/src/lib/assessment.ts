// Client-side mirror of the five-question AI Readiness Scorecard.
//
// The server (chat.crox.io) holds the canonical copy in
// `app/assessment_questions.py` and recomputes the score on every
// submission, so this file is only used to render the questions and
// give the visitor live feedback. Keep the two in sync when editing —
// they share question ids and option order (the server scores by
// option index, not by label).
